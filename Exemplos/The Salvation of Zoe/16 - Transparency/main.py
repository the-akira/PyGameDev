...
pygame.display.set_caption("The Salvation of Zoe")

# Carrega os tiles
tile_images = {
    "2": pygame.image.load("assets/Tiles/grass.png").convert_alpha(),
    "3": pygame.image.load("assets/Tiles/rock.png").convert_alpha(),
}

# Carrega os cristais coletáveis e portas
collectables = []
collectable_image = pygame.image.load("assets/Artifacts/crystal.png").convert_alpha()

doors = []
door_image = pygame.image.load("assets/Artifacts/door.png").convert_alpha()

# Carrega os planos de fundos e frases
background0_img = pygame.image.load("assets/Background/background0.png").convert_alpha()
background1_img = pygame.image.load("assets/Background/background1.png").convert_alpha()
background2_img = pygame.image.load("assets/Background/background2.png").convert_alpha()
...
# Função para desenhar um círculo transparente
def draw_transparent_circle(surface, color, center, radius, alpha):
    if radius > 0:
        ...
        # Desenhe o círculo na superfície transparente
        pygame.draw.circle(transparent_surface, color + (alpha,), (radius, radius), radius)
        # Blit a superfície transparente na superfície principal
...
crack_fx = pygame.mixer.Sound('sounds/crack.wav')
crack_fx.set_volume(0.15)

laugh_fx = pygame.mixer.Sound('sounds/laugh.wav')
laugh_fx.set_volume(0.15)
...
# Efeito de transição quando há troca de mapa
...
# Classe que representa as partículas da jogo
class Particle:
...
# Classe que representa o inimigo Caveira
class Skull(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.images = []
        for i in range(0,4):
            image = pygame.image.load(f"assets/Enemies/Skull/{i}.png")
            self.images.append(image)
        self.image = self.images[self.index]
        self.rect = self.image.get_rect()
        self.rect.x = random.randint(160, 1600)  # Posição X aleatória dentro dos limites
        self.rect.y = random.randint(0, 800)  # Posição Y aleatória
        self.y_speed = 1
    ...
    def update(self):
        ...
        # Remova o inimigo quando ele sair da tela
        if self.rect.x < 0:
            self.kill()
...
# Classe que representa o inimigo Olho
class Eye(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.images = []
        for i in range(0,9):
            image = pygame.image.load(f"assets/Enemies/Eye/{i}.png")
            self.images.append(image)
        self.image = self.images[self.index]
        self.rect = self.image.get_rect()
        self.rect.x = random.randint(160, 1600)  # Posição X aleatória dentro dos limites
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        for eye in eyes:
            if player.rect.colliderect(eye.rect):
                time_left -= 300
                laugh_fx.play()
                eyes.remove(eye)
...
# Classe que representa um item coletável
...
while running:
    ...
    if not main_menu and not game_over:
        ...
        if random.randint(0, 150) < 2:  # Ajuste a taxa de geração conforme necessário
            skull = Skull()
            skulls.add(skull)

        for skull in skulls:
            skull.update()
            skull.draw(camera_x, camera_y)

        if random.randint(0, 500) < 2:  # Ajuste a taxa de geração conforme necessário
            eye = Eye()
            eyes.add(eye)

        for eye in eyes:
            eye.update()
            eye.draw(camera_x, camera_y)
        ...
        if time_left > 0:
            ...
            draw_transparent_circle(screen, BLACK, (SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2), radius, 225)
    ...
    if game_over:
        ...
        screen.blit(background0_img,(0,0))
